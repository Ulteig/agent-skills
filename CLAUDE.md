...
To (re)link every skill into the local harness skill directories (`~/.claude/skills`, `~/.agents/skills`), run `scripts/link-skills.sh` on macOS/Linux or `powershell -NoProfile -File .\scripts\link-skills.ps1` on Windows. Each skill link points into this repo, so a `git pull` keeps installed skills current. Re-run the platform script after adding or renaming a skill; orphaned links are left in place and must be removed manually.
