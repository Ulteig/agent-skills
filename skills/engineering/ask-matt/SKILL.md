...
- **`/commit`** — turn one coherent staged or working-tree change into an atomic commit. It derives an intent-rich message from the patch, conversation, branch, linked work and local history, while preserving unrelated changes. Standalone: reach for it after work completed outside `/implement`, or whenever you have deliberately staged a follow-up.
- **`/pr-create`** — open a pull request from your current branch. It drafts a description from commit messages and the diff, shows it to you for edits, then creates the PR via `gh pr create`. Standalone: reach for it after committing work that is ready for review.
- **`/pr-review`** — review-only analysis of an existing GitHub PR by number. Checks SOLID, DRY, security, and best practices across every changed file. Standalone: reach for it when a PR exists and needs vetting before merge.
