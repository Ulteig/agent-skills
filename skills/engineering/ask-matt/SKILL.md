...
3. **Branch: is this a multi-session build?**
   ...
   Either way, **`/implement`** builds each issue by driving **`/tdd`** internally (one red-green slice at a time), then closes out by running **`/code-review`**, a two-axis review (Standards + Spec) of the diff, before committing. After the commit, use **`/pr-create`** to open a pull request with a description derived from the branch's commits and diff. Reach for **`/tdd`** on its own when you just want to build a concrete behaviour test-first without a full spec, **`/code-review`** on its own whenever you want to review a branch or PR against a fixed point, and **`/pr-review`** to review an existing GitHub PR.
...
- **`/commit`** turns one coherent staged or working-tree change into an atomic commit. It derives an intent-rich message from the patch, conversation, branch, linked work and local history, while preserving unrelated changes. Standalone: reach for it after work completed outside `/implement`, or whenever you have deliberately staged a follow-up.
- **`/pr-create`** opens a pull request from your current branch. It drafts a description from commit messages and the diff, shows it to you for edits, then creates the PR via `gh pr create`. Standalone: reach for it after committing work that is ready for review.
- **`/pr-review`** performs review-only analysis of an existing GitHub PR by number. It checks SOLID, DRY, security, and best practices across every changed file. Standalone: reach for it when a PR exists and needs vetting before merge.
